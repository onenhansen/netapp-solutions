...
. Update a `gridsetup.rsp` file for silent installation and place the rsp file in the `/tmp/archive` directory. The rsp file should cover sections A, B, and G with the following infomation:
+
....
...
. As the Oracle user, from the $GRID_HOME, create the LOGS disk group.
+
....
...
. Update the DB silent install standard rsp file in `/tmp/archive/dbinstall.rsp` directory with the following values in the relevant sections:
+
....
...
. As Oracle user, update the standard `dbca.rsp` file with the following entries in the relevant sections:
+
....
...
. As Oracle user, from $ORACLE_HOME directory, lauch DB creation with dbca.
+
....
